...
export default class BufferedStreamLoader {
	 ...
	 */
	private static downloadBuffer(url: string | undefined, redirectCount: number = 0, maxRedirects = 3): Promise<Buffer> {
		return new Promise((resolvePromise, rejectPromise) => {
            ...
            if (!url) {rejectPromise(`Undefined URL`); return;}; //## added undefined handler

			const parsed = parse(url);
			if (parsed.protocol !== 'https:') reject(new Error(`Invalid URL: ${url}`));

			const req = https.get(parsed, res => {
				if ([301, 302, 303, 307].indexOf(res.statusCode ??-1) > -1) {
				...
				} else if ((res.statusCode ?? 201) < 200 || (res.statusCode ?? 299) >= 300) {
